...
export function formatReviewComment(reviews, meta = {}) {
  const { repo = "unknown/repo", prNumber = "?", reviewedAt = new Date().toISOString(), similarPastIssues = [] } = meta;
      ...
      similarPastIssues?.length > 0
        ? "\n\n---\n\n## 📚 Similar past issues\n\n" +
          similarPastIssues
            .map(
              (s) =>
                `• **Similar issue:** ${s.severity} — ${s.title}. ${s.description ? s.description + " " : ""}(Seen in ${(s.sourceRefs || []).join(", ") || "past reviews"})`
            )
            .join("\n\n")
        : "";
    return `${header}
...
This diff looks clean across all ${reviews.length} chunk${reviews.length !== 1 ? "s" : ""}. Great work!${similarNote}`;
  }
...
\`\`\``;
  ...
  // ── Similar past issues (from RAG, deduped) ─────────────────────────────────
...
\`\`\`
  ...
  if (similarSection) parts.push("---", similarSection);
